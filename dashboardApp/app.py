...
db_string = f"postgres://{cfg.db_user}:{cfg.db_password}@{cfg.endpoint}:5432/{cfg.db_name}"
engine = create_engine(db_string)
zip_df = pd.read_sql_table(table_name="encoded_data",
                           con=engine, columns=["ZipCode"])
...
def formpost():
    ...
    if (not re.match("941\\d{2}", zip_code)):
        ...
        input_variables = pd.DataFrame([[day_of_week, time_of_day, police_district, crime_category, zip_code]], columns=[
                                       'Dow', 'Tod', 'district', 'category', 'zip'], dtype=int)
